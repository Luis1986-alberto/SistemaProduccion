...
{
    public partial class Frm_TipoMaterial : Form
    {
        ...
        public Frm_TipoMaterial()
        {
            InitializeComponent();
        }

        private void Frm_TipoMaterial_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoMaterial"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            dgv_Mnt.AutoGenerateColumns = false;
            List<MP_aTipoMaterial> lista = MP_aTipoMaterial_CN._Instancia.Lista_TipoMateriales().ToList();
            SortableBindingList<MP_aTipoMaterial> Lsta_tipomaterial = new SortableBindingList<MP_aTipoMaterial>(lista);

            dgv_Mnt.DataSource = Lsta_tipomaterial;
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTipoMaterial.Text = "0";
            txt_Descripcion.Text = "";
            txt_Abreviatura.Text = "";
            txt_CodigoSustrato.Text = "";
            Nud_Orden_Gerencia.Value = 0;

            txt_Descripcion.Enabled = true;
            txt_Abreviatura.Enabled = true;
            txt_CodigoSustrato.Enabled = true;
            Nud_Orden_Gerencia.Enabled = true;
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                string rpta = MP_aTipoMaterial_CN._Instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoMaterial"].Value.ToString()));
        ...
        private void tls_Grabar_Click(object sender, EventArgs e)
        {
            ...
            var datos = new MP_aTipoMaterial()
            {
                IdTipoMaterial = byte.Parse(txt_IdTipoMaterial.Text),
                Descripcion = txt_Descripcion.Text,
                Codigo_Sustrato = txt_CodigoSustrato.Text,
                Abreviatura = txt_Abreviatura.Text,
                Orden_Gerencia = byte.Parse(Nud_Orden_Gerencia.Value.ToString()),
            };

            if (Nuevo) rpta = MP_aTipoMaterial_CN._Instancia.Agregar(datos);
            if (Editar) rpta = MP_aTipoMaterial_CN._Instancia.Actualizar(datos);
            ...
            txt_CodigoSustrato.Enabled = false;
            Nud_Orden_Gerencia.Enabled = false;
            tbc_Mnt.SelectTab(1);
        ...
        private bool Verificar_Datos()
        {
            ...
            else { ErrorIcono.SetError(txt_Descripcion, null); }

            if (txt_Abreviatura.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Nombre Estado", "Ingreso de datos", MessageBoxButtons.OK);
                ErrorIcono.SetError(txt_Abreviatura, "Ingrese Abreviatura");
                return false;
            }
            else { ErrorIcono.SetError(txt_Abreviatura, null); }

            if (txt_CodigoSustrato.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Nombre Estado", "Ingreso de datos", MessageBoxButtons.OK);
                ErrorIcono.SetError(txt_CodigoSustrato, "Ingrese Codigo Sustrato");
                return false;
            }
            else { ErrorIcono.SetError(txt_CodigoSustrato, null); }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in MP_aTipoMaterial_CN._Instancia.Lista_TipoMateriales().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in MP_aTipoMaterial_CN._Instancia.Lista_TipoMateriales().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
            dgv_Mnt.DataSource = Listado_Ordenado;
        }
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Materiales ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Materiales ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
