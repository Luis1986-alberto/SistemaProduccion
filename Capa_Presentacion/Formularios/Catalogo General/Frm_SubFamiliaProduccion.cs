...
{
    public partial class Frm_SubFamiliaProduccion : Form
    {
        ...
        public Frm_SubFamiliaProduccion()
        {
            InitializeComponent();
        }

        private void Frm_SubFamiliaProduccion_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdSubFamiliaProd"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aSubFamilia_Produccion> Listado = PR_aSubFamilia_Produccion_CN._Instancia.Lista_subfamiliaProd();

            SortableBindingList<PR_aSubFamilia_Produccion> ListadoTipoRubro = new SortableBindingList<PR_aSubFamilia_Produccion>(Listado);
        ...
        {
            var result = PR_aSubFamilia_Produccion_CN._Instancia.TraerPorID(id);
            foreach (var i in result)
            {
                txt_IdSubFamiliaProd.Text = i.IdSubFamiliaProd.ToString();
                txt_Descripcion_SubFamilia.Text = i.Descripcion_SubFamiliaProd;
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdSubFamiliaProd.Text = "0";
            txt_Descripcion_SubFamilia.Text = "";
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aSubFamilia_Produccion_CN._Instancia.Eliminar_SubFamiliaProduccion(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdSubFamiliaProd"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aSubFamilia_Produccion()
            {
                IdSubFamiliaProd = Int32.Parse(txt_IdSubFamiliaProd.Text),
                Descripcion_SubFamiliaProd = txt_Descripcion_SubFamilia.Text,
                Observacion = txt_Observacion.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aSubFamilia_Produccion_CN._Instancia.Agregar_SubFamiliaProduccion(datos); }
            if (bln_Editar) { rpta = PR_aSubFamilia_Produccion_CN._Instancia.Actualizar_SubFamiliaProduccion(datos); }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Ordenar in PR_aSubFamilia_Produccion_CN._Instancia.Lista_subfamiliaProd().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Ordenar in PR_aSubFamilia_Produccion_CN._Instancia.Lista_subfamiliaProd().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Ordenar).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdSubFamiliaProd"].Value.ToString()));
        ...
        {
            if (txt_Descripcion_SubFamilia.Text.Trim() == string.Empty)
            {
                MessageBox.Show("Faltan Alfunos Datos", "Mensaje Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ErrorIcono.SetError(txt_Descripcion_SubFamilia, "Ingrese Descripcion SubFamilia");
                return false;
            }
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte SubFamilias";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de SubFamilia Produccion", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de SubFamilia Produccion", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
