...
{
    public partial class Frm_LineaProduccion : Form
    {
        ...
        public Frm_LineaProduccion()
        {
            InitializeComponent();
        }

        private void Frm_LineaProduccion_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
        ...
        private void tbc_Mnt_selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdlineaProd"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aLinea_Produccion> Lista = PR_aLinea_Produccion_CN.Instancia.Lista_LineaProduccion();
            SortableBindingList<PR_aLinea_Produccion> linea_produccions = new SortableBindingList<PR_aLinea_Produccion>(Lista);
            dgv_Mnt.DataSource = linea_produccions;
        }

        private void Entrada_Datos(byte id)
        {
            var datos = PR_aLinea_Produccion_CN.Instancia.TraerPorID(id);
            foreach (var i in datos)
            {
                txt_IdLineaProduccion.Text = i.IdLineaProd.ToString();
                txt_LineaProduccion.Text = i.Descripcion_Linea;
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_LineaProduccion.Enabled = true;
            txt_LineaProduccion.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aLinea_Produccion_CN._Instancia.Eliminar_LineaProduccion(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdLineaProd"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aLinea_Produccion()
            {
                IdLineaProd = byte.Parse(txt_IdLineaProduccion.Text),
                Descripcion_Linea = txt_LineaProduccion.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aLinea_Produccion_CN._Instancia.Agregar_LineaProduccion(datos); }
            if (bln_Editar) { rpta = PR_aLinea_Produccion_CN._Instancia.Actualizar_LineaProduccion(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_LineaProduccion.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aLinea_Produccion_CN._Instancia.Lista_LineaProduccion().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aLinea_Produccion_CN._Instancia.Lista_LineaProduccion().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
        ...
        {
            if (txt_LineaProduccion.Text.Trim() == string.Empty)
            {
                MessageBox.Show("Faltan Alfunos Datos", "Mensaje Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ErrorIcono.SetError(txt_LineaProduccion, "Ingrese Linea Produccion");
                return false;
            }
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Linea Produccion";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Linea Produccion ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Linea Produccion ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
