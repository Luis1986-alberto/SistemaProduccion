...
{
    public partial class Frm_EstadoCalificacionCC : Form
    {
        ...
        public Frm_EstadoCalificacionCC()
        {
            InitializeComponent();
        }

        private void Frm_EstadoCalificacionCC_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdEstadoCalificacion_CC"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aEstadoCalificacion_CC> Listado = PR_aEstadoCalificacion_CC_CN._Instancia.Lista_EstadoCalificacion_CC();
            SortableBindingList<PR_aEstadoCalificacion_CC> lst_estado = new SortableBindingList<PR_aEstadoCalificacion_CC>(Listado);
        ...
        {
            var result = PR_aEstadoCalificacion_CC_CN._Instancia.TraerPorID(idpestado);
            foreach (var i in result)
            {
                txt_IdEstadoCalificacion.Text = i.IdEstadoCalificacion_CC.ToString();
                txt_NombreEstadoCalificacion.Text = i.Nombre_EstadoCaficacion_CC.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdEstadoCalificacion.Text = "0";
            txt_NombreEstadoCalificacion.Text = "";

            txt_NombreEstadoCalificacion.Enabled = true;
            txt_NombreEstadoCalificacion.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aEstadoCalificacion_CC_CN._Instancia.Eliminar_EstadoCalificacionCC(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdEstadoCalificacion_CC"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aEstadoCalificacion_CC()
            {
                IdEstadoCalificacion_CC = byte.Parse(txt_IdEstadoCalificacion.Text),
                Nombre_EstadoCaficacion_CC = txt_NombreEstadoCalificacion.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aEstadoCalificacion_CC_CN._Instancia.Agregar_EstadoCalificacionCC(datos); }
            if (bln_Editar) { rpta = PR_aEstadoCalificacion_CC_CN._Instancia.Actualizar_EstadoCalificacionCC(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                Estado_Toolbar(bln_Editar);
                txt_NombreEstadoCalificacion.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_NombreEstadoCalificacion.Text.Trim() == "")
            {
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aEstadoCalificacion_CC_CN._Instancia.Lista_EstadoCalificacion_CC().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aEstadoCalificacion_CC_CN._Instancia.Lista_EstadoCalificacion_CC().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Estado Calificacion CC ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Estado Calificacion CC", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
