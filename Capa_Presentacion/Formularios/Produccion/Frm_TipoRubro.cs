...
namespace Capa_Presentacion.Formularios
{
    public partial class Frm_TipoRubro : Form
    {
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiporubro"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aTipoRubro> Listado = PR_aTipoRubro_CN._instancia.Lista_TipoRubro();

            SortableBindingList<PR_aTipoRubro> ListadoTipoRubro = new SortableBindingList<PR_aTipoRubro>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoTipoRubro;
        }

        private void Entrada_Datos(byte idtiporubro)
        {
            var result = PR_aTipoRubro_CN._instancia.TraerPorId(idtiporubro);
            foreach (var i in result)
            {
                txt_IdTipoRubro.Text = i.IdTipoRubro.ToString();
                txt_nombretiporubro.Text = i.Nombre_TipoRubro.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTipoRubro.Text = "0";
            txt_nombretiporubro.Text = "";
            txt_nombretiporubro.Enabled = true;
            txt_nombretiporubro.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoRubro_CN._instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiporubro"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoRubro()
            {
                IdTipoRubro = byte.Parse(txt_IdTipoRubro.Text),
                Nombre_TipoRubro = txt_nombretiporubro.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoRubro_CN._instancia.Agregar(datos); }
            if (bln_Editar) { rpta = PR_aTipoRubro_CN._instancia.Actualizar(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretiporubro.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretiporubro.Text.Trim() == "")
            {
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tiporubro in PR_aTipoRubro_CN._instancia.Lista_TipoRubro().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tiporubro).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tiporubro in PR_aTipoRubro_CN._instancia.Lista_TipoRubro().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tiporubro).ToList();
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Rubro", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Rubro", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
