...
{
    public partial class Frm_FormaSustrato : Form
    {
        DataGridViewPrinter dgr_Visor_Grilla;
        private bool bln_Nuevo = false;
        private bool bln_Editar = false;
        private string str_Campo;
        public Frm_FormaSustrato()
        {
            InitializeComponent();
        }

        private void Frm_FormaSustrato_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdFormaSustrato"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aFormaSustrato> Listado = PR_aFormaSustrato_CN._Instancia.Lista_FormaSustrato();

            SortableBindingList<PR_aFormaSustrato> lst = new SortableBindingList<PR_aFormaSustrato>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = lst;
        }

        private void Entrada_Datos(int idforma)
        {
            var result = PR_aFormaSustrato_CN._Instancia.TraerPorID(idforma);
            foreach (var i in result)
            {
                txt_IdFormaSustrato.Text = i.IdFormaSustrato.ToString();
                txt_NombreForma.Text = i.Descripcion_Forma.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdFormaSustrato.Text = "0";
            txt_NombreForma.Text = "";
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aFormaSustrato_CN._Instancia.Eliminar_FormaSustrato(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdFormaSustrato"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aFormaSustrato()
            {
                IdFormaSustrato = byte.Parse(txt_IdFormaSustrato.Text),
                Descripcion_Forma = txt_NombreForma.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aFormaSustrato_CN._Instancia.Agregar_FormaSustrato(datos); }
            if (bln_Editar) { rpta = PR_aFormaSustrato_CN._Instancia.Actualizar_FormaSustrato(datos); }
        ...
        {
            if (txt_NombreForma.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese la forma ", "Ingreso de datos", MessageBoxButtons.OK);
                txt_NombreForma.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aFormaSustrato_CN._Instancia.Lista_FormaSustrato().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aFormaSustrato_CN._Instancia.Lista_FormaSustrato().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdFormaSustrato"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte FormaSustrato";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Forma Sustrato", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Forma Sustrato", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
