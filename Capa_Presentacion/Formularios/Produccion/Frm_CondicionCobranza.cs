...
{
    public partial class Frm_CondicionCobranza : Form
    {
        ...
        public Frm_CondicionCobranza()
        {
            InitializeComponent();
        }

        private void Frm_CondicionPago_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        {
            List<LG_aCondicionCobranza> Listado = LG_aCondicionCobranza_CN._Instancia.Lista_CondicionCobranza();

            SortableBindingList<LG_aCondicionCobranza> ListadoMes = new SortableBindingList<LG_aCondicionCobranza>(Listado);
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            TxtIdCondicionCobranza.Text = "0";
            TxtCondicion_Cobranza.Text = "";
            TxtCondicion_Cobranza.Enabled = true;
            TxtCondicion_Cobranza.Focus();
        }
        ...
        {
            string rpta = "";
            var datos = new LG_aCondicionCobranza()
            {
                IdCondicionCobranza = byte.Parse(TxtIdCondicionCobranza.Text),
                Condicion_Cobranza = TxtCondicion_Cobranza.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = LG_aCondicionCobranza_CN._Instancia.Agregar_CondicionCobranza(datos); }
            if (bln_Editar) { rpta = LG_aCondicionCobranza_CN._Instancia.Actualizar_CondicionCobranza(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                TxtCondicion_Cobranza.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (TxtCondicion_Cobranza.Text.Trim() == "")
            {
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in LG_aCondicionCobranza_CN._Instancia.Lista_CondicionCobranza().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in LG_aCondicionCobranza_CN._Instancia.Lista_CondicionCobranza().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Condicion Cobranza", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Condicion Cobranza", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);

            return true;
        }
        private void txt_Nombre_CondicionPago_Enter(object sender, EventArgs e)
        {
            TxtCondicion_Cobranza.BackColor = Color.Cornsilk;
        }

        private void txt_Nombre_CondicionPago_Leave(object sender, EventArgs e)
        {
            TxtCondicion_Cobranza.BackColor = Color.White;
        }
