...
{
    public partial class Frm_TipoTinta : Form
    {
        ...
        public Frm_TipoTinta()
        {
            InitializeComponent();
        }

        private void Frm_TipoTinta_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotinta"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aTipoTinta> Listado = PR_aTipoTinta_CN._Instancia.Lista_TipoTinta();

            SortableBindingList<PR_aTipoTinta> lst_tipotinta = new SortableBindingList<PR_aTipoTinta>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = lst_tipotinta;
        }

        private void Entrada_Datos(int idtipotinta)
        {
            var result = PR_aTipoTinta_CN._Instancia.TraerPorID(idtipotinta);
            foreach (var i in result)
            {
                txt_IdTipotinta.Text = i.IdTipoTinta.ToString();
                txt_nombretipotinta.Text = i.Descripcion_TipoTinta.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTipotinta.Text = "0";
            txt_nombretipotinta.Text = "";
            txt_nombretipotinta.Enabled = true;
            txt_nombretipotinta.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoTinta_CN._Instancia.Eliminar_TipoTinta(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotinta"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoTinta()
            {
                IdTipoTinta = byte.Parse(txt_IdTipotinta.Text),
                Descripcion_TipoTinta = txt_nombretipotinta.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoTinta_CN._Instancia.Agregar_TipoTinta(datos); }
            if (bln_Editar) { rpta = PR_aTipoTinta_CN._Instancia.Actualizar_TipoTinta(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretipotinta.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretipotinta.Text.Trim() == "")
            {
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipotinta in PR_aTipoTinta_CN._Instancia.Lista_TipoTinta().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipotinta).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipotinta in PR_aTipoTinta_CN._Instancia.Lista_TipoTinta().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipotinta).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotinta"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Tipo Tinta";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Tinta", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Tinta", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
