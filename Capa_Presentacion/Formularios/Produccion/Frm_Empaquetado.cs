...
{
    public partial class Frm_Empaquetado : Form
    {
        ...
        public Frm_Empaquetado()
        {
            InitializeComponent();
        }

        private void Frm_Empaquetado_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdEmpaquetado"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aEmpaquetado> Listado = PR_aEmpaquetado_CN._Instancia.Lista_Empaquetado();

            SortableBindingList<PR_aEmpaquetado> ListadoMes = new SortableBindingList<PR_aEmpaquetado>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoMes;
        }

        private void Entrada_Datos(byte idempaquetado)
        {
            var result = PR_aEmpaquetado_CN._Instancia.TraerPorID(idempaquetado);
            foreach (var i in result)
            {
                txt_IdEmpaquetado.Text = i.IdEmpaquetado.ToString();
                txt_Nombre_Empaquetado.Text = i.Descripcion.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdEmpaquetado.Text = "0";
            txt_Nombre_Empaquetado.Text = "";
            txt_Nombre_Empaquetado.Enabled = true;
            txt_Nombre_Empaquetado.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aEmpaquetado_CN._Instancia.Eliminar_Empaquetado(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdEmpaquetado"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aEmpaquetado()
            {
                IdEmpaquetado = byte.Parse(txt_IdEmpaquetado.Text),
                Descripcion = txt_Nombre_Empaquetado.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aEmpaquetado_CN._Instancia.Agregar_Empaquetado(datos); }
            if (bln_Editar) { rpta = PR_aEmpaquetado_CN._Instancia.Actualizar_Empaquetado(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                Estado_Toolbar(bln_Editar);
                txt_Nombre_Empaquetado.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_Nombre_Empaquetado.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el tipo empaquetado", "Ingreso de datos", MessageBoxButtons.OK);
                txt_Nombre_Empaquetado.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aEmpaquetado_CN._Instancia.Lista_Empaquetado().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aEmpaquetado_CN._Instancia.Lista_Empaquetado().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Empaquetados";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Empaquetado", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Empaquetado", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);

            return true;
        }

        private void txt_Nombre_Empaquetado_Enter(object sender, EventArgs e)
        {
            txt_Nombre_Empaquetado.BackColor = Color.Cornsilk;
        }

        private void txt_Nombre_Empaquetado_Leave(object sender, EventArgs e)
        {
            txt_Nombre_Empaquetado.BackColor = Color.White;
        }
    }
}
