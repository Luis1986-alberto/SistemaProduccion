...
{
    public partial class Frm_TipoFuelle : Form
    {
        ...
        public Frm_TipoFuelle()
        {
            InitializeComponent();
        }

        private void Frm_Fuelle_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoFuelle"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aTipoFuelle> Listado = PR_aTipoFuelle_CN._Instancia.Lista_TipoFuelle();

            SortableBindingList<PR_aTipoFuelle> Lista = new SortableBindingList<PR_aTipoFuelle>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = Lista;
        }

        private void Entrada_Datos(int idfuelle)
        {
            var result = PR_aTipoFuelle_CN._Instancia.TraerPorID(idfuelle);
            foreach (var i in result)
            {
                txt_IdFuelle.Text = i.IdTipoFuelle.ToString();
                txt_NombreFuelle.Text = i.Descripcion_TipoFuelle.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdFuelle.Text = "0";
            txt_NombreFuelle.Text = "";
            txt_NombreFuelle.Enabled = true;
            txt_NombreFuelle.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoFuelle_CN._Instancia.Eliminar_TipoFuelle(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoFuelle"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoFuelle()
            {
                IdTipoFuelle = byte.Parse(txt_IdFuelle.Text),
                Descripcion_TipoFuelle = txt_NombreFuelle.Text,
            };
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_NombreFuelle.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aTipoFuelle_CN._Instancia.Lista_TipoFuelle().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aTipoFuelle_CN._Instancia.Lista_TipoFuelle().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoFuelle"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipos De Fuelles", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipos de Fuelle", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
