...
namespace Capa_Presentacion.Formularios
{
    public partial class Frm_Rodillo : Form
    {
        ...
        private void Frm_Rodillo_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdRodillo"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aRodillos> Listado = PR_aRodillo_CN._Instancia.Lista_Rodillos();

            SortableBindingList<PR_aRodillos> lst = new SortableBindingList<PR_aRodillos>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = lst;
        }

        private void Entrada_Datos(int idrodillo)
        {
            var result = PR_aRodillo_CN._Instancia.TraerPorID(idrodillo);
            foreach (var i in result)
            {
                txt_IdRodillo.Text = i.IdRodillo.ToString();
                txt_Rodillo.Text = i.Descripcion.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdRodillo.Text = "0";
            txt_Rodillo.Text = "";
            txt_Rodillo.Enabled = true;
            txt_Rodillo.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aRodillo_CN._Instancia.Eliminar_Rodillos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdRodillo"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aRodillos()
            {
                IdRodillo = byte.Parse(txt_IdRodillo.Text),
                Descripcion = txt_Rodillo.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aRodillo_CN._Instancia.Agregar_Rodillos(datos); }
            if (bln_Editar) { rpta = PR_aRodillo_CN._Instancia.Actualizar_Rodillos(datos); }
        ...
        {
            if (txt_Rodillo.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el tipo Pestaña", "Ingreso de datos", MessageBoxButtons.OK);
                txt_Rodillo.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from rodillos in PR_aRodillo_CN._Instancia.Lista_Rodillos().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select rodillos).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from rodillos in PR_aRodillo_CN._Instancia.Lista_Rodillos().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select rodillos).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdRodillo"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Rodillos", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Rodillo", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
