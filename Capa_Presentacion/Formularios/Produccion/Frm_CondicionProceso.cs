...
{
    public partial class Frm_CondicionProceso : Form
    {
        ...
        public Frm_CondicionProceso()
        {
            InitializeComponent();
        }

        private void Frm_CondicionProceso_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        {
            List<PR_aCondicionProceso> Listado = PR_aCondicionProceso_CN._Instancia.Lista_CondicionProceso();

            SortableBindingList<PR_aCondicionProceso> listacondicionproc = new SortableBindingList<PR_aCondicionProceso>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = listacondicionproc;
        }

        private void Entrada_Datos(byte idcondicionproceso)
        {
            var result = PR_aCondicionProceso_CN._Instancia.TraerID(idcondicionproceso);
            foreach (var i in result)
            {
                txt_IdCondicionProceso.Text = i.IdCondicionProceso.ToString();
                txt_Nombre_CondicionProceso.Text = i.Nombre_CondicionProceso.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdCondicionProceso.Text = "0";
            txt_Nombre_CondicionProceso.Text = "";
            txt_Nombre_CondicionProceso.Enabled = true;
            txt_Nombre_CondicionProceso.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aCondicionProceso_CN._Instancia.Eliminar_CondicionProceso(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdCondicionProceso"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aCondicionProceso()
            {
                IdCondicionProceso = byte.Parse(txt_IdCondicionProceso.Text),
                Nombre_CondicionProceso = txt_Nombre_CondicionProceso.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aCondicionProceso_CN._Instancia.Agregar_CondicionProceso(datos); }
            if (bln_Editar) { rpta = PR_aCondicionProceso_CN._Instancia.Actualizar_CondicionProceso(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_Nombre_CondicionProceso.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_Nombre_CondicionProceso.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Color", "Ingreso de datos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txt_Nombre_CondicionProceso.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from condicionProc in PR_aCondicionProceso_CN._Instancia.Lista_CondicionProceso().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select condicionProc).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from condicionProc in PR_aCondicionProceso_CN._Instancia.Lista_CondicionProceso().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select condicionProc).ToList();
        ...
        private void txt_Nombre_CondicionProceso_Enter(object sender, EventArgs e)
        {
            txt_Nombre_CondicionProceso.BackColor = Color.Cornsilk;
        }

        private void txt_Nombre_CondicionProceso_Leave(object sender, EventArgs e)
        {
            txt_Nombre_CondicionProceso.BackColor = Color.White;
        }
    }
}
