...
{
    public partial class Frm_SeVendePor : Form
    {
        ...
        public Frm_SeVendePor()
        {
            InitializeComponent();
        }

        private void Frm_SeVendePor_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdSeVendePor"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aSeVendePor> Listado = PR_aSeVendePor_CN._Instancia.Lista_SeVendePor();

            SortableBindingList<PR_aSeVendePor> Listadosevende = new SortableBindingList<PR_aSeVendePor>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = Listadosevende;
        }

        private void Entrada_Datos(byte idsevendepor)
        {
            var result = PR_aSeVendePor_CN._Instancia.TraerPor_Id(idsevendepor);
            txt_IdSeVendePor.Text = result.IdSeVendePor.ToString();
            txt_SeVendePor.Text = result.SeVende_Por.ToString();            
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdSeVendePor.Text = "0";
            txt_SeVendePor.Text = "";
            txt_SeVendePor.Enabled = true;
            txt_SeVendePor.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aSeVendePor_CN._Instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdSEVendePor"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aSeVendePor()
            {
                IdSeVendePor = byte.Parse(txt_IdSeVendePor.Text),
                SeVende_Por = txt_SeVendePor.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aSeVendePor_CN._Instancia.Agregar(datos); }
            if (bln_Editar) { rpta = PR_aSeVendePor_CN._Instancia.Actualizar(datos); }
        ...
        {
            if (txt_SeVendePor.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese descripcion ", "Ingreso de datos", MessageBoxButtons.OK);
                txt_SeVendePor.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aSeVendePor_CN._Instancia.Lista_SeVendePor().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Se Vende Por";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Se Vende Por", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Se Vende Por", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
