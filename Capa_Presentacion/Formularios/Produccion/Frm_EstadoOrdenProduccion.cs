...
namespace Capa_Presentacion.Formularios
{
    public partial class Frm_EstadoOrdenProduccion : Form
    {
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdEstado_OP"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aEstado_OP> Listado = PR_aEstado_OP_CN.Instancia.Lista_Estado_OP();
            SortableBindingList<PR_aEstado_OP> Lst_EstadoOP = new SortableBindingList<PR_aEstado_OP>(Listado);
            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = Lst_EstadoOP;
        }

        private void Entrada_Datos(byte idestado)
        {
            var datos = PR_aEstado_OP_CN.Instancia.TraerPorID(idestado);
            foreach (var i in datos)
            {
                txt_IdEstadoOP.Text = i.IdEstado_OP.ToString();
                txt_descripcion_OP.Text = i.Descripcion_EstadoOP.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_descripcion_OP.Enabled = true;
            txt_descripcion_OP.Text = "";
            txt_IdEstadoOP.Text = "0";
            txt_descripcion_OP.Focus();
        }
        ...
        private void tls_Grabar_Click(object sender, EventArgs e)
        {
            ...
            var datos = new PR_aEstado_OP()
            {
                IdEstado_OP = byte.Parse(txt_IdEstadoOP.Text),
                Descripcion_EstadoOP = txt_descripcion_OP.Text,
            };

            if (bln_Nuevo) rpta = PR_aEstado_OP_CN.Instancia.Agregar_EstadoOP(datos);
            if (bln_Editar) rpta = PR_aEstado_OP_CN.Instancia.Actualizar_EstadoOP(datos);
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_descripcion_OP.Enabled = false;
                txt_IdEstadoOP.Enabled = false;
        ...
        {
            if (txt_descripcion_OP.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Estado de la OP", "Ingreso de datos", MessageBoxButtons.OK);
                txt_descripcion_OP.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aEstado_OP_CN._Instancia.Lista_Estado_OP().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aEstado_OP_CN._Instancia.Lista_Estado_OP().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Estado OP";
