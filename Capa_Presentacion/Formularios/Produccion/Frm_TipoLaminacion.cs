...
{
    public partial class Frm_TipoLaminacion : Form
    {
        DataGridViewPrinter dgr_Visor_Grilla;
        private bool bln_Editar, bln_Nuevo = false;
        private string str_Campo;

        public Frm_TipoLaminacion()
        {
            InitializeComponent();
        }

        private void Frm_TipoLaminacion_Load(object sender, EventArgs e)
        {
            Cargar_Datos();
            tbc_Mnt.SelectTab(1);
        ...
        private void Cargar_Datos()
        {
            ...
            List<PR_aTipoLaminacion> Listado = PR_aTipoLaminacion_CN.Instancia.Lista_TipoLaminacion();
            SortableBindingList<PR_aTipoLaminacion> Lista_Laminacion = new SortableBindingList<PR_aTipoLaminacion>(Listado);
            dgv_Mnt.DataSource = Lista_Laminacion;
        ...
        private void Entrada_Datos(byte idtipolaminacion)
        {
            ...
            foreach (var i in datos)
            {
                ...
                txt_NotaLaminacion.Text = i.Nota_Laminacion.ToString();
            }
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoLaminacion_CN._Instancia.Eliminar_TipoLaminacion(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoLaminado"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoLaminacion()
            {
                IdTipoLaminado = byte.Parse(txt_IdTipoLaminacion.Text),
                Nota_Laminacion = txt_NotaLaminacion.Text,
                Detalle_Laminacion = txt_TipoLaminacion.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoLaminacion_CN._Instancia.Agregar_TipoLaminacion(datos); }
            if (bln_Editar) { rpta = PR_aTipoLaminacion_CN._Instancia.Actualizar_TipoLaminacion(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_NotaLaminacion.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aTipoLaminacion_CN._Instancia.Lista_TipoLaminacion().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aTipoLaminacion_CN._Instancia.Lista_TipoLaminacion().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        {
            if (txt_NotaLaminacion.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese Nota Laminacion", "Ingreso de datos", MessageBoxButtons.OK);
                txt_NotaLaminacion.Focus();
                return false;
            }
            ...
            {
                MessageBox.Show("Ingrese tipo laminacion", "Ingreso de datos", MessageBoxButtons.OK);
                txt_TipoLaminacion.Focus();
                return false;
            }
