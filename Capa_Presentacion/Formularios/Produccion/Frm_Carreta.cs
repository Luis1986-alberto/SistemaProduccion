...
{
    public partial class Frm_Carreta : Form
    {
        ...
        private void Frm_Carreta_Load(object sender, EventArgs e)
        {
            Cargar_Datos();
            tbc_Mnt.SelectTab(1);
        ...
        private void Cargar_Datos()
        {
            ...
            List<PR_aCarreta> Listado = PR_aCarreta_CN._Instancia.Lista_Carretas();

            SortableBindingList<PR_aCarreta> ListadoCarreta = new SortableBindingList<PR_aCarreta>(Listado);
        ...
        private void Entrada_Datos(byte vidcarreta)
        {
            ...
            {
                txt_IdCarreta.Text = Registro.IdCarreta.ToString();
                Nud_PesoCarreta.Text = Registro.Pesos_Kilos.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            Nud_PesoCarreta.Enabled = true;
            Nud_PesoCarreta.Text = "0.00";
            txt_IdCarreta.Text = "0";
            Estado_Toolbar(bln_Nuevo);

            txt_IdCarreta.Enabled = true;
            Nud_PesoCarreta.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aCarreta_CN._Instancia.Eliminar_Carreta(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdCarreta"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aCarreta()
            {
                IdCarreta = byte.Parse(txt_IdCarreta.Text),
                Pesos_Kilos = decimal.Parse(Nud_PesoCarreta.Text),
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aCarreta_CN._Instancia.Agregar_Carreta(datos); }
            if (bln_Editar) { rpta = PR_aCarreta_CN._Instancia.Actualizar_Carreta(datos); }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aCarreta_CN._Instancia.Lista_Carretas().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aCarreta_CN._Instancia.Lista_Carretas().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
