...
{
    public partial class Frm_MarcaMaquina : Form
    {
        ...
        public Frm_MarcaMaquina()
        {
            InitializeComponent();
        }

        private void Frm_MarcaMaquina_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idmarcamaquina"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aMarcaMaquina> Listado = PR_aMarcaMaquina_CN._Instancia.Lista_MarcaMaquina();

            SortableBindingList<PR_aMarcaMaquina> ListadoMes = new SortableBindingList<PR_aMarcaMaquina>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoMes;
        }

        private void Entrada_Datos(int idmarcamaquina)
        {
            var result = PR_aMarcaMaquina_CN._Instancia.TraerPorID(idmarcamaquina);
            foreach (var i in result)
            {
                txt_IdMarcaMaquina.Text = i.IdMarcaMaquina.ToString();
                txt_NombreMarca.Text = i.Descripcion_MarcaMaquina.ToString();
                txt_Abreviaturamarcamaquina.Text = i.Abreviatura_MarcaMaquina.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdMarcaMaquina.Text = "0";
            txt_NombreMarca.Text = "";
            txt_Abreviaturamarcamaquina.Text = "";
            txt_NombreMarca.Enabled = true;
            txt_Abreviaturamarcamaquina.Enabled = true;
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
               string rpta = PR_aMarcaMaquina_CN._Instancia.Eliminar_MarcaMaquina(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdMarcaMaquina"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aMarcaMaquina()
            {
                IdMarcaMaquina = byte.Parse(txt_IdMarcaMaquina.Text),
                Descripcion_MarcaMaquina = txt_NombreMarca.Text,
                Abreviatura_MarcaMaquina = txt_Abreviaturamarcamaquina.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aMarcaMaquina_CN._Instancia.Agregar_MarcaMaquina(datos); }
            if (bln_Editar) { rpta = PR_aMarcaMaquina_CN._Instancia.Actualizar_MarcaMaquina(datos); }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Marcas in PR_aMarcaMaquina_CN._Instancia.Lista_MarcaMaquina().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Marcas).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from marcas in PR_aMarcaMaquina_CN._Instancia.Lista_MarcaMaquina().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select marcas).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdMarcaMaquina"].Value.ToString()));
        ...
        {
            if (txt_NombreMarca.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese la marca ", "Ingreso de datos", MessageBoxButtons.OK);
                txt_NombreMarca.Focus();
                return false;
            }
            return true;
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Marca Maquina";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Marcas de Maquina", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Marcas de Maquinas", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
