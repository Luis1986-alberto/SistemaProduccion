...
{
    public partial class Frm_TipoTrabajador : Form
    {
        ...
        public Frm_TipoTrabajador()
        {
            InitializeComponent();
        }

        private void Frm_TipoTrabajador_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotrabajador"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aTipoTrabajador> Listado = PR_aTipoTrabajador_CN._Instancia.Lista_TipoTrabajador();

            SortableBindingList<PR_aTipoTrabajador> ListadoTipoTrabajador = new SortableBindingList<PR_aTipoTrabajador>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoTipoTrabajador;
        }

        private void Entrada_Datos(int idtipotrabajador)
        {
            var result = PR_aTipoTrabajador_CN._Instancia.TraerPorID(idtipotrabajador);
            foreach (var i in result)
            {
                txt_IdTipoTrabajador.Text = i.IdTipoTrabajador.ToString();
                txt_nombretipotrabajador.Text = i.Nombre_TipoTrabajador.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTipoTrabajador.Text = "0";
            txt_nombretipotrabajador.Text = "";
            txt_nombretipotrabajador.Enabled = true;
            txt_nombretipotrabajador.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoTrabajador_CN._Instancia.Eliminar_TipoTrabajador(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotrabajador"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoTrabajador()
            {
                IdTipoTrabajador = byte.Parse(txt_IdTipoTrabajador.Text),
                Nombre_TipoTrabajador = txt_nombretipotrabajador.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoTrabajador_CN._Instancia.Agregar_TipoTrabajador(datos); }
            if (bln_Editar) { rpta = PR_aTipoTrabajador_CN._Instancia.Actualizar_TipoTrabajador(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretipotrabajador.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretipotrabajador.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el tipo de tinta", "Ingreso de datos", MessageBoxButtons.OK);
                txt_nombretipotrabajador.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipotrabajador in PR_aTipoTrabajador_CN._Instancia.Lista_TipoTrabajador().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipotrabajador).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipotrabajador in PR_aTipoTrabajador_CN._Instancia.Lista_TipoTrabajador().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipotrabajador).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotrabajador"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Tipo Trabajador";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo trabajador", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo trabajador", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
