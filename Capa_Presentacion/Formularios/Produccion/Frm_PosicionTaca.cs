...
{
    public partial class Frm_PosicionTaca : Form
    {
        ...
        public Frm_PosicionTaca()
        {
            InitializeComponent();
        }

        private void Frm_PosicionTaca_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdPosiciontaca"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aPosicionTaca> Listado = PR_aPosicionTaca_CN._Instancia.Lista_PosicionTaca();

            SortableBindingList<PR_aPosicionTaca> listadopieimp = new SortableBindingList<PR_aPosicionTaca>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = listadopieimp;
        }

        private void Entrada_Datos(byte idposiciontaca)
        {
            var result = PR_aPosicionTaca_CN._Instancia.Lista_TraerPorId(idposiciontaca);
            foreach (var i in result)
            {
                txt_IdPosicionTaca.Text = i.IdPosicionTaca.ToString();
                txt_PosicionTaca.Text = i.Posicion_Taca.ToString();
            }
        }
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdPosicionTaca.Text = "0";
            txt_PosicionTaca.Text = "";
            txt_PosicionTaca.Enabled = true;
            txt_PosicionTaca.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aPosicionTaca_CN._Instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdPosicionTaca"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aPosicionTaca()
            {
                IdPosicionTaca = byte.Parse(txt_IdPosicionTaca.Text),
                Posicion_Taca = txt_PosicionTaca.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aPosicionTaca_CN._Instancia.Agregar(datos); }
            if (bln_Editar) { rpta = PR_aPosicionTaca_CN._Instancia.Actualizar(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_PosicionTaca.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from pieimprenta in PR_aPosicionTaca_CN._Instancia.Lista_PosicionTaca().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select pieimprenta).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from posiciontaca in PR_aPosicionTaca_CN._Instancia.Lista_PosicionTaca().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select posiciontaca).ToList();
        ...
        {
            if (txt_PosicionTaca.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Posicion Taca", "Ingreso de datos", MessageBoxButtons.OK);
                txt_PosicionTaca.Focus();
                return false;
            }
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Posicion Taca", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Posicion Taca", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
