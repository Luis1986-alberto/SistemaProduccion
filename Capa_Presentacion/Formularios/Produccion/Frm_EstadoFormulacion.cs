...
{
    public partial class Frm_EstadoFormulacion : Form
    {
        DataGridViewPrinter dgr_Visor_Grilla;
        private bool Nuevo, editar = false;
        private string str_Campo;

        public Frm_EstadoFormulacion()
        {
            InitializeComponent();
        }

        private void Frm_EstadoFormulacion_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void Cargar_Datos()
        {
            ...
            List<PR_aEstadoFormulacion> Lista = PR_aEstadoFormulacion_CN.Instancia.Lista_EstadoFormulacion();
            SortableBindingList<PR_aEstadoFormulacion> Lista_EstForm = new SortableBindingList<PR_aEstadoFormulacion>(Lista);
        ...
        {
            var datos = PR_aEstadoFormulacion_CN.Instancia.TraerPorID(id);
            foreach (var i in datos)
            {
                txt_IdEstadoFormulacion.Text = i.IdEstadoFormulacion.ToString();
                txt_NombreEstadoformulacion.Text = i.Nombre_EstadoFormulacion.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_NombreEstadoformulacion.Enabled = true;
            Estado_Toolbar(Nuevo);

            txt_IdEstadoFormulacion.Text = "0";
            txt_NombreEstadoformulacion.Text = "";
            txt_NombreEstadoformulacion.Focus();
        }
        ...
        private void tls_Grabar_Click(object sender, EventArgs e)
        {
            ...
            var datos = new PR_aEstadoFormulacion()
            {
                IdEstadoFormulacion = byte.Parse(txt_IdEstadoFormulacion.Text),
                Nombre_EstadoFormulacion = txt_NombreEstadoformulacion.Text,
            };
            ...
            txt_NombreEstadoformulacion.Enabled = false;
            tbc_Mnt.SelectTab(1);
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aEstadoFormulacion_CN._Instancia.Lista_EstadoFormulacion().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        {
            str_Campo = dgv_Mnt.Columns[dgv_Mnt.CurrentCell.ColumnIndex].Name;
            var Listado_Ordenado = (from ordenar in PR_aEstadoFormulacion_CN._Instancia.Lista_EstadoFormulacion().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
            dgv_Mnt.DataSource = Listado_Ordenado;
        }
