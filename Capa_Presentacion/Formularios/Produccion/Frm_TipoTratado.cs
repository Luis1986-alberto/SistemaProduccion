...
{
    public partial class Frm_TipoTratado : Form
    {
        ...
        public Frm_TipoTratado()
        {
            InitializeComponent();
        }

        private void Frm_Tratado_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotratado"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aTipoTratado> Listado = PR_aTipoTratado_CN._Instancia.Lista_TipoTratado();

            SortableBindingList<PR_aTipoTratado> ListadoTipoRubro = new SortableBindingList<PR_aTipoTratado>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoTipoRubro;
        }

        private void Entrada_Datos(int idtratado)
        {
            var result = PR_aTipoTratado_CN._Instancia.TraerPorID(idtratado);
            foreach (var i in result)
            {
                txt_IdTratado.Text = i.IdTipoTratado.ToString();
                txt_nombretratado.Text = i.Tipo_Tratado.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTratado.Text = "0";
            txt_nombretratado.Text = "";
            txt_nombretratado.Enabled = true;
            txt_nombretratado.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoTratado_CN._Instancia.Eliminar_TipoTratado(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoTratado"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoTratado()
            {
                IdTipoTratado = byte.Parse(txt_IdTratado.Text),
                Tipo_Tratado = txt_nombretratado.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoTratado_CN._Instancia.Agregar_TipoTratado(datos); }
            if (bln_Editar) { rpta = PR_aTipoTratado_CN._Instancia.Actualizar_TipoTratado(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretratado.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretratado.Text.Trim() == "")
            {
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tratado in PR_aTipoTratado_CN._Instancia.Lista_TipoTratado().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tratado).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tratado in PR_aTipoTratado_CN._Instancia.Lista_TipoTratado().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tratado).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotratado"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Tipo Tratado";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Tratado", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Tratado", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
