...
namespace Capa_Presentacion.Formularios
{
    public partial class Frm_Estado : Form
    {
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdEstado"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aEstado> Listado = PR_aEstado_CN._Instancia.Lista_Estado();

            SortableBindingList<PR_aEstado> lst_estado = new SortableBindingList<PR_aEstado>(Listado);
        ...
        {
            var result = PR_aEstado_CN._Instancia.TraerPorID(idpestado);
            foreach (var i in result)
            {
                txt_IdEstado.Text = i.IdEstado.ToString();
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aEstado_CN._Instancia.Eliminar(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdEstado"].Value.ToString()));
        ...
        private void tls_Grabar_Click(object sender, EventArgs e)
        {
            ...
            if (bln_Nuevo) { rpta = PR_aEstado_CN._Instancia.Agregar(datos); }
            if (bln_Editar) { rpta = PR_aEstado_CN._Instancia.Actualizar(datos); }
        ...
        {
            if (txt_NombreEstado.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Estado.", "Ingreso de datos", MessageBoxButtons.OK);
                txt_NombreEstado.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Estado in PR_aEstado_CN._Instancia.Lista_Estado().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from EstadoOP in PR_aEstado_CN._Instancia.Lista_Estado().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Estado ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Estado ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
