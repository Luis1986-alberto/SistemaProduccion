...
{
    public partial class Frm_TipoTroquel : Form
    {
        ...
        public Frm_TipoTroquel()
        {
            InitializeComponent();
        }

        private void Frm_Troquel_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotroquel"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aTipoTroquel> Listado = PR_aTipoTroquel_CN._Instancia.Lista_TipoTroquel();

            SortableBindingList<PR_aTipoTroquel> ListadoTipoTroquel = new SortableBindingList<PR_aTipoTroquel>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoTipoTroquel;
        }

        private void Entrada_Datos(int idtroquel)
        {
            var result = PR_aTipoTroquel_CN._Instancia.TraerPorID(idtroquel);
            foreach (var i in result)
            {
                txt_IdTroquel.Text = i.IdTipoTroquel.ToString();
                txt_nombretroquel.Text = i.Descripcion_TipoTroquel.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTroquel.Text = "0";
            txt_nombretroquel.Text = "";
            txt_nombretroquel.Enabled = true;
            txt_nombretroquel.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoTroquel_CN._Instancia.Eliminar_TipoTroquel(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotroquel"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoTroquel()
            {
                IdTipoTroquel = byte.Parse(txt_IdTroquel.Text),
                Descripcion_TipoTroquel = txt_nombretroquel.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoTroquel_CN._Instancia.Agregar_Tipotroquel(datos); }
            if (bln_Editar) { rpta = PR_aTipoTroquel_CN._Instancia.Actualizar_TipoTroquel(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretroquel.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretroquel.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el nombre troquel", "Ingreso de datos", MessageBoxButtons.OK);
                txt_nombretroquel.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from troquel in PR_aTipoTroquel_CN._Instancia.Lista_TipoTroquel().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from troquel in PR_aTipoTroquel_CN._Instancia.Lista_TipoTroquel().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select troquel).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtroquel"].Value.ToString()));
        ...
        private void tls_Anterior_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipotroquel"].Value.ToString()));
        }
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Tratado";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Troquel", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Troquel", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
