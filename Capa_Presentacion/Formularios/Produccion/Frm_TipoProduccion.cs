...
{
    public partial class Frm_TipoProduccion : Form
    {
        ...
        public Frm_TipoProduccion()
        {
            InitializeComponent();
        }

        private void Frm_TipoProduccion_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipoproduccion"].Value.ToString())); }
        }
        private void Cargar_Datos()
        {
            List<PR_aTipoProduccion> Listado = PR_aTipoProduccion_CN._Instancia.Lista_TipoProduccion();

            SortableBindingList<PR_aTipoProduccion> ListadoMes = new SortableBindingList<PR_aTipoProduccion>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoMes;
        }

        private void Entrada_Datos(int idtipoproduccion)
        {
            var result = PR_aTipoProduccion_CN._Instancia.TraerPorID(idtipoproduccion);
            foreach (var i in result)
            {
                txt_IdTipoProduccion.Text = i.IdTipoProduccion.ToString();
                txt_nombretipoproduccion.Text = i.Detalle_TipoProduccion.ToString();
                txt_SiglasTipoProduccion.Text = i.Siglas_TipoProduccion.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTipoProduccion.Text = "0";
            txt_nombretipoproduccion.Text = "";
            txt_nombretipoproduccion.Enabled = true;
            txt_SiglasTipoProduccion.Enabled = true;
            txt_nombretipoproduccion.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoProduccion_CN._Instancia.Eliminar_TipoProduccion(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipoproduccion"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoProduccion()
            {
                IdTipoProduccion = byte.Parse(txt_IdTipoProduccion.Text),
                Detalle_TipoProduccion = txt_nombretipoproduccion.Text,
                Siglas_TipoProduccion = txt_SiglasTipoProduccion.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoProduccion_CN._Instancia.Agregar_TipoProduccion(datos); }
            if (bln_Editar) { rpta = PR_aTipoProduccion_CN._Instancia.Actualizar_TipoProduccion(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretipoproduccion.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretipoproduccion.Text.Trim() == "")
            {
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipoproduccion in PR_aTipoProduccion_CN._Instancia.Lista_TipoProduccion().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipoproduccion).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipoproduccion in PR_aTipoProduccion_CN._Instancia.Lista_TipoProduccion().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipoproduccion).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtipoproduccion"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Produccion", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Produccion", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
