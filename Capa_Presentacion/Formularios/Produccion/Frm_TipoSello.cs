...
{
    public partial class Frm_TipoSello : Form
    {
        ...
        public Frm_TipoSello()
        {
            InitializeComponent();
        }

        private void Frm_TipoSello_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiposello"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            dgv_Mnt.AutoGenerateColumns = false;
            List<PR_aTipoSello> Listado = PR_aTipoSello_CN._Instancia.Lista_TipoSello();
            SortableBindingList<PR_aTipoSello> ListadoTipoRubro = new SortableBindingList<PR_aTipoSello>(Listado);

            dgv_Mnt.DataSource = ListadoTipoRubro;
        }

        private void Entrada_Datos(int idtiposello)
        {
            var result = PR_aTipoSello_CN._Instancia.TraerPorID(idtiposello);
            foreach (var i in result)
            {
                txt_IdTipoSello.Text = i.IdTipoSello.ToString();
                txt_nombretiposello.Text = i.Descripcion_TipoSello.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTipoSello.Text = "0";
            txt_nombretiposello.Text = "";
            txt_nombretiposello.Enabled = true;
            txt_nombretiposello.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoSello_CN._Instancia.Eliminar_TipoSello(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiposello"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoSello()
            {
                IdTipoSello = byte.Parse(txt_IdTipoSello.Text),
                Descripcion_TipoSello = txt_nombretiposello.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoSello_CN._Instancia.Agregar_TipoSello(datos); }
            if (bln_Editar) { rpta = PR_aTipoSello_CN._Instancia.Actualizar_TipoSello(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretiposello.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretiposello.Text.Trim() == "")
            {
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tiposello in PR_aTipoSello_CN._Instancia.Lista_TipoSello().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tiposello).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tiposello in PR_aTipoSello_CN._Instancia.Lista_TipoSello().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tiposello).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiposello"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Tipo Sello";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Sello", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Sello", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
