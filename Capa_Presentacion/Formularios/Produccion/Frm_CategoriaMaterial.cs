...
{
    public partial class Frm_CategoriaMaterial : Form
    {
        ...
        public Frm_CategoriaMaterial()
        {
            InitializeComponent();
        }

        private void Frm_CategoriaMaterial_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdCategoriaMaterial"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            dgv_Mnt.AutoGenerateColumns = false;
            List<MP_aCategoriaMaterial> lista = MP_aCategoriaMaterial_CN._Instancia.Lista_CategoriaMaterial();
            SortableBindingList<MP_aCategoriaMaterial> Lsta_etiquetadoras = new SortableBindingList<MP_aCategoriaMaterial>(lista);
        ...
        {
            var datos = MP_aCategoriaMaterial_CN._Instancia.TraerPorID(Id);

            foreach (var i in datos)
            {
                txt_IdCategoriaMaterial.Text = i.IdCategoriaMaterial.ToString();
                txt_Nombre_Categoria_Material.Text = i.Nombre_Categoria_Material;
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdCategoriaMaterial.Text = "0";
            txt_Nombre_Categoria_Material.Text = "";

            txt_Nombre_Categoria_Material.Enabled = true;
            txt_Nombre_Categoria_Material.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                string rpta = MP_aCategoriaMaterial_CN._Instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdCategoriaMaterial"].Value.ToString()));
        ...
        private void tls_Grabar_Click(object sender, EventArgs e)
        {
            ...
            var datos = new MP_aCategoriaMaterial()
            {
                IdCategoriaMaterial = byte.Parse(txt_IdCategoriaMaterial.Text),
                Nombre_Categoria_Material = txt_Nombre_Categoria_Material.Text,
            };

            if (Nuevo) rpta = MP_aCategoriaMaterial_CN._Instancia.Agregar(datos);
            if (Editar) rpta = MP_aCategoriaMaterial_CN._Instancia.Actualizar(datos);
            ...
            txt_Nombre_Categoria_Material.Enabled = false;
            tbc_Mnt.SelectTab(1);
        ...
        {
            if (txt_Nombre_Categoria_Material.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Nombre Estado", "Ingreso de datos", MessageBoxButtons.OK);
                ErrorIcono.SetError(txt_Nombre_Categoria_Material, "Ingrese Estado Formulacion");
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in MP_aCategoriaMaterial_CN._Instancia.Lista_CategoriaMaterial().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in MP_aCategoriaMaterial_CN._Instancia.Lista_CategoriaMaterial().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
            dgv_Mnt.DataSource = Listado_Ordenado;
        }
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Categoria Material ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Categoria Material", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
