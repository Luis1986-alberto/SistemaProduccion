...
namespace Capa_Presentacion.Formularios.Produccion
{
    public partial class Frm_TipoVenta : Form
    {
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoVenta"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<PR_aTipoVenta> Listado = PR_aTipoVenta_CN._Instancia.Lista_TipoVenta();

            SortableBindingList<PR_aTipoVenta> Listatipoventa = new SortableBindingList<PR_aTipoVenta>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = Listatipoventa;
        }

        private void Entrada_Datos(byte idtipoventa)
        {
            var result = PR_aTipoVenta_CN._Instancia.TraerPorId(idtipoventa);
            txt_idtipoventa.Text = result.IdTipoVenta.ToString();
            txt_tipoventa.Text = result.TipoVenta.ToString();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = PR_aTipoVenta_CN._Instancia.Eliminar_TipoVenta(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoVenta"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new PR_aTipoVenta()
            {
                IdTipoVenta = byte.Parse(txt_idtipoventa.Text),
                TipoVenta = txt_tipoventa.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = PR_aTipoVenta_CN._Instancia.Agregar_TipoVenta(datos); }
            if (bln_Editar) { rpta = PR_aTipoVenta_CN._Instancia.Actualizar_TipoVenta(datos); }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from Años in PR_aTipoVenta_CN._Instancia.Lista_TipoVenta().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select Años).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in PR_aTipoVenta_CN._Instancia.Lista_TipoVenta().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        {
            if (txt_tipoventa.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el tipo Venta", "Ingreso de datos", MessageBoxButtons.OK);
                txt_tipoventa.Focus();
                return false;
            }
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte TipoVenta";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Venta", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Venta", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
