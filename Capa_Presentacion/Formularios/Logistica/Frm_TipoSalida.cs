...
{
    public partial class Frm_TipoSalida : Form
    {
        ...
        public Frm_TipoSalida()
        {
            InitializeComponent();
        }

        private void Frm_TipoSalida_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiposalida"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<LG_aTipoSalida> Listado = LG_aTipoSalida_CN._Instancia.Lista_TipoSalida();

            SortableBindingList<LG_aTipoSalida> ListadoTipoSalida = new SortableBindingList<LG_aTipoSalida>(Listado);

            dgv_Mnt.AutoGenerateColumns = false;
            dgv_Mnt.DataSource = ListadoTipoSalida;
        }

        private void Entrada_Datos(int idtiposalida)
        {
            var result = LG_aTipoSalida_CN._Instancia.TraerPorID(idtiposalida);
            foreach (var i in result)
            {
                txt_IdTipoSalida.Text = i.IdTipoSalida.ToString();
                txt_nombretiposalida.Text = i.Nombre_TipoSalida.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdTipoSalida.Text = "0";
            txt_nombretiposalida.Text = "";
            txt_nombretiposalida.Enabled = true;
            txt_nombretiposalida.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = LG_aTipoSalida_CN._Instancia.Eliminar_TipoSalida(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiposalida"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new LG_aTipoSalida()
            {
                IdTipoSalida = byte.Parse(txt_IdTipoSalida.Text),
                Nombre_TipoSalida = txt_nombretiposalida.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = LG_aTipoSalida_CN._Instancia.Agregar_TipoSalida(datos); }
            if (bln_Editar) { rpta = LG_aTipoSalida_CN._Instancia.Actualizar_TipoSalida(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                txt_nombretiposalida.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        {
            if (txt_nombretiposalida.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el tipo de rubro", "Ingreso de datos", MessageBoxButtons.OK);
                txt_nombretiposalida.Focus();
                return false;
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tiporsalida in LG_aTipoSalida_CN._Instancia.Lista_TipoSalida().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tiporsalida).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tiposalida in LG_aTipoSalida_CN._Instancia.Lista_TipoSalida().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tiposalida).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["idtiposalida"].Value.ToString()));
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Reporte Tipo Rubro";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Salida", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Salida", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
