...
{
    public partial class Frm_FormaPago : Form
    {
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdFormaPago"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<LG_aFormaPago> Listado = LG_aFormaPago_CN._Instancia.Lista_FormaPago();
            SortableBindingList<LG_aFormaPago> listadopieimp = new SortableBindingList<LG_aFormaPago>(Listado);
        ...
        {
            var result = LG_aFormaPago_CN._Instancia.TraerPorID(id);
            txt_IdFormaPago.Text = result.IdFormaPago.ToString();
            Txt_Nombre_FormaPago.Text = result.Nombre_FormaPago.ToString();
            Nud_Dias.Value = result.Dias;
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdFormaPago.Text = "0";
            Txt_Nombre_FormaPago.Text = "";
            Txt_Nombre_FormaPago.Enabled = true;
            Nud_Dias.Enabled = true;
            Txt_Nombre_FormaPago.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = LG_aFormaPago_CN._Instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdFormaPago"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new LG_aFormaPago()
            {
                IdFormaPago = byte.Parse(txt_IdFormaPago.Text),
                Nombre_FormaPago = Txt_Nombre_FormaPago.Text,
                Dias = Int16.Parse(Nud_Dias.Value.ToString()),
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = LG_aFormaPago_CN._Instancia.Agregar(datos); }
            if (bln_Editar) { rpta = LG_aFormaPago_CN._Instancia.Actualizar(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                Txt_Nombre_FormaPago.Enabled = false;
                Nud_Dias.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in LG_aFormaPago_CN._Instancia.Lista_FormaPago().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in LG_aFormaPago_CN._Instancia.Lista_FormaPago().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdFormaPago"].Value.ToString()));
        ...
        {
            if (Txt_Nombre_FormaPago.Text == string.Empty)
            {
                MessageBox.Show("Ingrese Nombre Forma Pago", "Ingreso de datos", MessageBoxButtons.OK);
                Txt_Nombre_FormaPago.Focus();
                return false;
            }
            if (Nud_Dias.Value == 0)
            {
                MessageBox.Show("Ingrese Numero de Dias", "Ingreso de datos", MessageBoxButtons.OK);
                Txt_Nombre_FormaPago.Focus();
                return false;
            }
            return true;
