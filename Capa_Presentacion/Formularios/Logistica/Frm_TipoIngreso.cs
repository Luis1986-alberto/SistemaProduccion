...
namespace Capa_Presentacion.Formularios
{
    public partial class Frm_TipoIngreso : Form
    {
        ...
        private void Frm_TipoIngreso_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoIngreso"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<LG_aTipoIngreso> Listado = LG_aTipoIngreso_CN._Instancia.Lista_TipoIngreso();

            SortableBindingList<LG_aTipoIngreso> listadopieimp = new SortableBindingList<LG_aTipoIngreso>(Listado);
        ...
        {
            var result = LG_aTipoIngreso_CN._Instancia.TraerPorID(idtipoingreso);
            foreach (var i in result)
            {
                txt_IdPieImprenta.Text = i.IdTipoIngreso.ToString();
                Txt_NombreTipoIngreso.Text = i.Nombre_TipoIngreso.ToString();
            }
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdPieImprenta.Text = "0";
            Txt_NombreTipoIngreso.Text = "";
            Txt_NombreTipoIngreso.Enabled = true;
            Txt_NombreTipoIngreso.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = LG_aTipoIngreso_CN._Instancia.Eliminar_TipoIngreso(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoingreso"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new LG_aTipoIngreso()
            {
                IdTipoIngreso = byte.Parse(txt_IdPieImprenta.Text),
                Nombre_TipoIngreso = Txt_NombreTipoIngreso.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = LG_aTipoIngreso_CN._Instancia.Agregar_TipoIngreso(datos); }
            if (bln_Editar) { rpta = LG_aTipoIngreso_CN._Instancia.Actualizar_TipoIngreso(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                Txt_NombreTipoIngreso.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipoingreso in LG_aTipoIngreso_CN._Instancia.Lista_TipoIngreso().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipoingreso).ToList();
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from tipoingreso in LG_aTipoIngreso_CN._Instancia.Lista_TipoIngreso().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select tipoingreso).ToList();
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdTipoIngreso"].Value.ToString()));
        ...
        {
            if (Txt_NombreTipoIngreso.Text.Trim() == "")
            {
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Tipo Ingreso ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Tipo Ingreso ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
