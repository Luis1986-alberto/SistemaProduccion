...
{
    public partial class Frm_CondicionPago : Form
    {
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdCondicionPago"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<LG_aCondicionPago> Listado = LG_aCondicionPago_CN._Instancia.Lista_CondicionPago();
            SortableBindingList<LG_aCondicionPago> listadopieimp = new SortableBindingList<LG_aCondicionPago>(Listado);
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdCondicionPago.Text = "0";
            Txt_Nombre_Condicion_Pago.Text = "";
            Txt_Nombre_Condicion_Pago.Enabled = true;
            Txt_Nombre_Condicion_Pago.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = LG_aCondicionPago_CN._Instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdCondicionPago"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new LG_aCondicionPago()
            {
                IdCondicionPago = byte.Parse(txt_IdCondicionPago.Text),
                Nombre_Condicion_Pago = Txt_Nombre_Condicion_Pago.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = LG_aCondicionPago_CN._Instancia.Agregar(datos); }
            if (bln_Editar) { rpta = LG_aCondicionPago_CN._Instancia.Actualizar(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                Txt_Nombre_Condicion_Pago.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdCondicionPago"].Value.ToString()));
        ...
        {
            if (Txt_Nombre_Condicion_Pago.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese la condicion Pago", "Ingreso de datos", MessageBoxButtons.OK);
                Txt_Nombre_Condicion_Pago.Focus();
                return false;
            }
        ...
        private bool SetupThePrinting()
        {
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Condicion Pago ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Condicion Pago ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
