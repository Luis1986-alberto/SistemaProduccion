...
{
    public partial class Frm_UbicacionInmueble : Form
    {
        ...
        public Frm_UbicacionInmueble()
        {
            InitializeComponent();
        }

        private void Frm_UbicacionInmueble_Load(object sender, EventArgs e)
        {
            tbc_Mnt.SelectTab(1);
            Cargar_Datos();
        ...
        private void tbc_Mnt_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            { Entrada_Datos(Int32.Parse(dgv_Mnt.SelectedRows[0].Cells["IdUbicacionInmueble"].Value.ToString())); }
        }

        private void Cargar_Datos()
        {
            List<LG_aUbicacionInmueble> Listado = LG_aUbicacionInmueble_CN._Instancia.Lista_UbicacionInmueble();
            SortableBindingList<LG_aUbicacionInmueble> listadopieimp = new SortableBindingList<LG_aUbicacionInmueble>(Listado);
        ...
        {
            var result = LG_aUbicacionInmueble_CN._Instancia.traerPorID(id);
            txt_IdUbicacionInmueble.Text = result.IdUbicacionInmueble.ToString();
            Txt_Nombre_Distrito.Text = result.Nombre_Distrito.ToString();
        }
        ...
        private void tls_Agregar_Click(object sender, EventArgs e)
        {
            ...
            txt_IdUbicacionInmueble.Text = "0";
            Txt_Nombre_Distrito.Text = "";
            Txt_Nombre_Distrito.Enabled = true;
            Txt_Nombre_Distrito.Focus();
        }
        ...
        private void tls_Eliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                rpta = LG_aUbicacionInmueble_CN._Instancia.Eliminar(byte.Parse(dgv_Mnt.SelectedRows[0].Cells["IdUbicacionInmueble"].Value.ToString()));
        ...
        {
            string rpta = "";
            var datos = new LG_aUbicacionInmueble()
            {
                IdUbicacionInmueble = byte.Parse(txt_IdUbicacionInmueble.Text),
                Nombre_Distrito = Txt_Nombre_Distrito.Text,
            };

            if (Verificar_Datos() == false) { return; }

            if (bln_Nuevo) { rpta = LG_aUbicacionInmueble_CN._Instancia.Agregar(datos); }
            if (bln_Editar) { rpta = LG_aUbicacionInmueble_CN._Instancia.Actualizar(datos); }
            ...
            if (rpta == "PROCESADO")
            {
                ...
                Txt_Nombre_Distrito.Enabled = false;
                tbc_Mnt.SelectTab(1);
            }
        ...
        private void tls_OrdenASC_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in LG_aUbicacionInmueble_CN._Instancia.Lista_UbicacionInmueble().OrderBy(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
        ...
        private void tls_OrdenDsc_Click(object sender, EventArgs e)
        {
            ...
            var Listado_Ordenado = (from ordenar in LG_aUbicacionInmueble_CN._Instancia.Lista_UbicacionInmueble().OrderByDescending(r => r.GetType().GetProperty(str_Campo).GetValue(r, null))
                                    select ordenar).ToList();
            dgv_Mnt.DataSource = Listado_Ordenado;
        }
        ...
        private void tls_Primero_Click(object sender, EventArgs e)
        {
            ...
            Entrada_Datos(int.Parse(dgv_Mnt.SelectedRows[0].Cells["IdUbicacionInmueble"].Value.ToString()));
        ...
        {
            if (Txt_Nombre_Distrito.Text.Trim() == "")
            {
                MessageBox.Show("Ingrese el Nombre Distrito ", "Ingreso de datos", MessageBoxButtons.OK);
                Txt_Nombre_Distrito.Focus();
                return false;
            }
        ...
        private bool SetupThePrinting()
        {
            ...
            PrintDocument.DocumentName = "Nombre Distrito";
            ...
            if (MessageBox.Show("Desea que el informe se centre en la página", "Estilo de Impresión", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, true, true, "Listado de Nombre Distrito ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
            else
                dgr_Visor_Grilla = new DataGridViewPrinter(dgv_Mnt, PrintDocument, false, true, "Listado de Nombre Distrito ", new Font("Tahoma", 18, FontStyle.Bold, GraphicsUnit.Point), Color.Black, true);
